    ...
    socket.on("answer:submit", (payload) => {
        ...
        if (room.mode === "hotseat") {
            ...
            if (socket.id === room.targetPlayerId) {
                ...
                const answered = answeredMap.get(key) ?? new Map();
            ...
            if (targetAnswer !== undefined) {
                ...
                const pending = pendingAnswerMap.get(key) ?? new Map();
        ...
        if (!player)
            return;
        const correctIndex = room.socialModeType === "set-answers-first" && room.hostAnswers !== undefined
...
const liveHostAnswerMap = new Map();
...
function recordQuestionStart(io, room) {
